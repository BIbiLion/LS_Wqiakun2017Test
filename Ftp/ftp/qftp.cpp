...
/*
    The QFtpDTP (DTP = Data Transfer sRes) controls all client side
...
void QFtpPI::readyRead()
{
    ...
    while (commandSocket.canReadLine()) {
        ...
        qDebug()<<"readRead socket:"<<line<<"\nCurret command:"<<currentCommand();
...
bool QFtpPI::processReply()
{
...
#endif
    qDebug()<<replyCodeInt<<"------------------"<<replyText<<currentCommand();
    if(replyText.startsWith("The system cannot find the file specified")&& ( replyCodeInt !=550))//wuqiankun
    {
    ...
    else if(replyCodeInt == 550)//wqiankun 上传
    {
        qDebug()<<"550 currentCmd"<<currentCmd;
        if (currentCmd.startsWith(QLatin1String("SIZE ")))
        {
            if(isConUpLoad)                                     //wuqiankun 20170414
            {
                m_rmtFileSize = 0;
                dtp.setBytesDone(0);
                dtp.data.dev->seek(0);
...
#endif
    state = Waiting;
    commandSocket.write(currentCmd.toLatin1());//.toStdString().c_str()//wuqiankun
    return true;
}
...
int QFtp::conPut(QIODevice *dev, const QString &file, TransferType type)
{
    ...
    if (type == Binary)
        ...
        cmds << QLatin1String("TYPE A\r\n");
    cmds << QLatin1String("SIZE ") +QString::fromLatin1( file.toLocal8Bit())+ QLatin1String("\r\n");
    cmds << QLatin1String(d->transferMode == Passive ? "PASV\r\n" : "PORT\r\n");
    if (!dev->isSequential())
        cmds << QLatin1String("ALLO ") + QString::number(dev->size()) + QLatin1String("\r\n");
    cmds<<  QLatin1String("APPE ") +QString::fromLatin1( file.toLocal8Bit())+ QLatin1String("\r\n");
   //以FTP服务器为 UTF8为例子
...
{
    return d->addCommand(new QFtpCommand(Remove, QStringList(QLatin1String("DELE ") +QString::fromLatin1(file.toLocal8Bit()) + QLatin1String("\r\n"))));
}
