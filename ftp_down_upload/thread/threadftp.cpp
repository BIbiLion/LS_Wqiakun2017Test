...
ObjectFtp::ObjectFtp(QObject *parent) :
    QObject(parent),m_flag(true),m_file(NULL),m_uploadfile(NULL)
{
    m_urlInfoList.clear();
    m_DirurlInfoList.clear();
    m_downFileUrlInfoList.clear();
...
void ObjectFtp::putFile(QString filePath, QString file)
{
    ...
    if(m_uploadfile !=NULL)
    {
        qDebug()<<"BBBBBBBBBBB  NOT EMPTY";
    }
    else
    {
        qDebug()<<"eeeeeeeeeeeeeee EMPTY";
    }


    m_uploadfile= new QFile(filePath);
    if( !m_uploadfile->open(QIODevice::ReadWrite))
    {
        qDebug()<<"not";
    }
...
void ObjectFtp::commandFinshed(int type,bool error)
{
    ...
    case QFtp::Get:
        ...
        {
            qDebug()<<"get gggggggggggggggggggg m_file delete";
            m_file->close();
            m_file->deleteLater();;
            m_file=NULL;
        }
    ...
    case QFtp::Put:
        ...
        if(m_uploadfile !=NULL && m_uploadfile->isOpen())
        {
            m_uploadfile->close();
            m_uploadfile->deleteLater();;
            m_uploadfile=NULL;
