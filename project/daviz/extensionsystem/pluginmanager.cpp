    ...
    : d(new PluginManagerPrivate(this))
{
    ...
    t->load(QLocale::system(), "ExtensionSystem", ".", ":/extensionsystem/translations");
...
{
    pluginPaths = paths;
    readSettings();//读取 setting配置信息 global local
    readPluginPaths();
}
...
void PluginManagerPrivate::readPluginPaths()
{
    ...
    while (!searchPaths.isEmpty()) {
        ...
        const QString pattern = QLatin1String("*.") + extension;//*.xml    *.pluginSpec
